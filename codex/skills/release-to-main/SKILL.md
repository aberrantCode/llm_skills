...
2. Resolve obvious ones yourself. For non-obvious conflicts, prompt the user:
   ```
   Ask the user: "Rebase conflict in <files>. How would you like to proceed?"
   Options:
   - "I'll resolve manually" — pause, let the user fix the files, then continue
   - "Abort the rebase" — run git rebase --abort and stop the workflow
   ```
...
Compute the three candidate versions (new_patch, new_minor, new_major) and present them to the user with the auto-detected bump pre-selected:

```
Ask the user: "What version should this release be tagged as?"
Options:
- "vX.Y.Z+1" — Patch bump (auto-detected from commits) (Recommended)
- "vX.Y+1.0" — Minor bump
- "vX+1.0.0" — Major bump
```
...
Show a summary panel to the user, then ask for confirmation:
...
Ask the user: "Merge dev into main as $VERSION?"
Options:
- "Yes — ship it" — Proceed with merge, tag, and dev sync
- "No — abort" — Stop the workflow without making any changes
```
...
| `LOCAL_AHEAD>0, LOCAL_BEHIND=0` | Local main has unpushed commits — ask the user (see below) |
| Both > 0 (diverged) | **STOP** — local and remote main have diverged; do not merge. Report to user and abort |

**When local main is ahead of origin/main**, ask the user:

```
Ask the user: "Local main has $LOCAL_AHEAD commit(s) not on origin/main:
<git log --oneline origin/main..HEAD>
How should these be handled?"
Options:
- "Push them to origin/main first" — Publish the local commits, then continue the release merge
- "Abort — I need to review these commits" — Stop the workflow without making any changes
```
...
if git rev-parse "$VERSION" >/dev/null 2>&1; then
  # Tag already exists — ask the user
  Ask the user: "Tag $VERSION already exists locally or on remote. How would you like to proceed?"
  Options:
  - "Pick a different version" — Go back and choose a new version string
  - "Abort" — Stop the workflow without making any further changes
fi
```
...
If rebase conflicts arise here (rare — just merged), use the same conflict-resolution prompt pattern from Step 2.
...
| Rebase conflict (Step 2 or 7) | Prompt user: resolve manually or abort |
| Local main ahead of origin/main | Ask the user: push local commits first, or abort |
...
| Tag already exists | Guard in Step 6 catches this — ask user: pick a different version or abort |
