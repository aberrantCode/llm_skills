...
AGENTS.md must reference session-management.md in the Skills section. The agent reads AGENTS.md first, which directs it to follow session rules.
...
2. Announce what was found: "Resuming from: [last state]"
3. Confirm next steps before proceeding

### 5. Periodic Self-Audit
Every ~20 tool calls, check:
...
- "What's the current state?" → Read and report state files
- "End session" → Triggers archive + handoff
- "Resume from last session" → Read state files first
