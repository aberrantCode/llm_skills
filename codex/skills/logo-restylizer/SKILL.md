...
**Both signals are required.** If either is clearly absent and cannot be inferred from recent context, ask the user — never ask inline as plain text. Ask only once, combining all missing questions into a single prompt.
...
- **Neon/glow effects** — add or modify `<filter>` with `feGaussianBlur` + `feMerge` nodes.
...
Ask the user to confirm the output location only if they seem likely to want a non-default path (e.g., they mentioned deploying to a specific location). Otherwise write to the default and state the path in your response.
...
| Situation | Action |
|-----------|--------|
| Source file not found | Use `Glob` to search; if still not found, ask the user |
| Transformation is ambiguous (e.g., "make it better") | Ask the user — what visual direction do they have in mind? |
| User asks to overwrite the original | Confirm with the user before doing so |
