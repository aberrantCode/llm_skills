---
name: workspace
description: Dynamic multi-repo and monorepo awareness - analyze workspace topology, track API contracts, and maintain cross-repo context
---

# Workspace Analysis Skill

> Dynamic multi-repo and monorepo awareness for the agent. Analyze workspace topology, track API contracts, and maintain cross-repo context.

---

## The Problem

When you have separate frontend/backend repos (or monorepo with multiple apps), the agent operates in isolation. It doesn't know:
...
- the agent reimplementing things that exist elsewhere

---

## Solution: Dynamic Workspace Analysis

Instead of static manifests that get stale, the agent dynamically analyzes the workspace and generates context artifacts that stay fresh through hooks.
...
Identify files the agent MUST know about for each context:
...
| the agent detects... | Load additionally |
|-------------------|-------------------|
...
the agent accesses via relative paths: `../myapp-backend/`
...
When the agent detects changes that affect other modules:
