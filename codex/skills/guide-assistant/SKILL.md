...
description: >
  ...
  path to a markdown document and asks for help following it. Always prompt the user interactively
  for every question — never just print a question as plain text.
---
...
**Critical rule**: You MUST prompt the user interactively for every question you ask.
Never print a question as plain text and wait. Every prompt is an interactive exchange.
...
If the user hasn't provided a file path, ask the user for it.
...
**If it is NOT a checklist**, ask the user:
...
Ask the user for the outcome. Options should be:
- "Done — it worked"
...
2. Ask the user where to record the resolution:
...
1. Ask the user: "Should I update the source file with corrected instructions?"
...
- **Interactive prompt for every user question.** No exceptions. Not even "just a quick check" as
  plain text.
...
- **Never mark a step complete** without the user explicitly confirming it.
