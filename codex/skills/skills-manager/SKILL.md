...
1. **Determine scope:**
   ...
   - If no `name`: scan current project's `.claude/skills/` for all skills that do NOT have `installed-from: llm_skills` and differ from the archive; prompt the user to confirm:
...
3. Check frontmatter: if `installed-from: llm_skills` is present, ask the user to confirm rejection:
