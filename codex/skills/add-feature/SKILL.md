...
Work through phases **in order**. Use interactive prompts for structured choices at each phase.
...
**Immediately and automatically** invoke the `visual-explainer:generate-web-diagram` skill —
do not ask for permission first. Pass this prompt:
...
If they say yes, invoke the `superpowers:writing-plans` skill.
