...
| Partial | Implementation exists but has no test, or test exists but logic is incomplete/incorrect |
...
| AU-CAP-09  | Unauthenticated UI guard ("Sign out") | Partial        | Logic present, no test         |
```
...
For every capability that is Partial or Missing, write a B-style task contract.
...
Present the gap table and the implementation plan to the user. Ask the user to choose one of:

- **Approved — begin implementation** — create a worktree and start Phase 6
- **Revise the plan first** — describe what to change
- **Stop here** — implement manually later
